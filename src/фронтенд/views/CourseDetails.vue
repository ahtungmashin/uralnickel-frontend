  ...
  <div class="course-details-page" v-if="course">
    ...
    <div class="hero-content">
      ...
      <div class="hero-info">
        <div class="meta-grid">
          <div class="meta-box">
            <div class="meta-label">Отделы:</div>
            <div class="meta-value-tags">
              <span v-for="(dept, i) in parsedDepartments" :key="i" class="tag">{{ dept }}</span>
            </div>
          </div>
          <div class="meta-box">
    ...
    <div class="enroll-wrapper">
      <button @click="enroll" class="btn-enroll" :disabled="loading">
...
export default {
  ...
  computed: {
    parsedDepartments() {
      const data = this.course?.departments || [];
  ...
  async created() {
    this.toast = useToast(); // ✅ инициализация
    ...
    try {
      ...
      this.toast.error('Ошибка при загрузке данных курса');
    }
  },
  methods: {
    formatDate(dateStr) {
    ...
    async enroll() {
      try {
        ...
        this.toast.success(res.data.message || 'Заявка успешно отправлена ✅');
      } catch (error) {
